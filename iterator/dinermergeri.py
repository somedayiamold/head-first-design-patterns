...
class Menu(metaclass=ABCMeta):
    ...
    def get_menu_items(self) -> List[MenuItem]:
        raise NotImplementedError('`get_menu_items` method not implemented')
...
class Waitress:
    ...
    def print_menu(self, with_new_constructs: int) -> None:
        if with_new_constructs:
            breakfast_items: List[MenuItem] = self.pancake_house_menu.get_menu_items()
            for m in breakfast_items:
                self.print_menu_item(m)

            lunch_items: List[MenuItem] = self.diner_menu.get_menu_items()
            for m in lunch_items:
                self.print_menu_item(m)
        else:
